...
function CalendarDayCell({
  ...
  return (
    <div
      className={`h-[52px] relative cursor-pointer rounded ${
        isSpecialDay && !isSelected ? "bg-slate-50" : ""
      }`}
      onClick={handleClick}
    ...
    >
      {/* Numéro du jour */}
      <div
        className={`absolute top-0 left-1/2 -translate-x-1/2 min-w-[28px] h-6 flex items-center justify-center text-xs rounded-full z-[1] ${dayNumClass}`}
      >
        {dayNum}
      </div>
      ...
      {placeholder && (
        <div
          className="absolute bottom-1 left-0 right-0 h-6 text-[11px] text-slate-500 font-medium flex items-center px-1.5 truncate bg-slate-200/80 border-2 border-dashed border-slate-400 z-[2]"
          style={{
            borderTopLeftRadius: isFirstOfPlaceholder ? 12 : 0,
            borderBottomLeftRadius: isFirstOfPlaceholder ? 12 : 0,
            borderTopRightRadius: isLastOfPlaceholder ? 12 : 0,
            borderBottomRightRadius: isLastOfPlaceholder ? 12 : 0,
            borderLeftWidth: isFirstOfPlaceholder ? 2 : 0,
            borderRightWidth: isLastOfPlaceholder ? 2 : 0,
      ...
      {/* Pivot: fin + début sur la même cellule */}
      {isPivot && endingEvent && startingEvent ? (
        <>
          <div
            className={`absolute bottom-1 left-0 w-1/2 h-6 z-[2] ${
          ...
          />
          <div
            className={`absolute bottom-1 right-0 w-1/2 h-6 text-[11px] text-white font-medium flex items-center px-1.5 truncate z-[2] ${
        ...
        <div
          className={`absolute bottom-1 left-0 right-0 h-6 text-[11px] text-white font-medium flex items-center px-1.5 truncate z-[2] ${
