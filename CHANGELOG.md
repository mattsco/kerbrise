...
- **#37 — Mode hors ligne (PWA)** (spec `docs/specs/pwa-offline-minimal.md`) : Kerbrise était une PWA **installée** sans service worker — réseau coupé, écran d'erreur du navigateur. Or c'est précisément pendant une panne de wifi qu'on a besoin des marées, des poubelles ou du mot de passe de la box. La surface hors ligne **calque l'architecture en ligne** (`/hors-ligne`, `/hors-ligne/calendrier`, `/hors-ligne/a-propos`, `/hors-ligne/a-propos/regles`) : mêmes cartes, même ordre, un bandeau qui explique pourquoi les sections grisées attendent le réseau.
  - **Service worker écrit à la main** (`app/sw.js/route.ts`, < 150 lignes, zéro dépendance), servi par une route pour que le nom du cache dérive du commit Vercel — impossible d'oublier de bumper une constante. Il ne met **jamais** en cache une page vivante de l'app : toute navigation part sur le réseau, le cache ne sert qu'en secours. C'est ce qui rend le scénario « la famille voit un vieux dashboard » structurellement impossible, et ce qui permet de se passer d'un prompt de mise à jour.
  - **Précache post-login uniquement**, et **purge complète à la déconnexion** (caches + snapshot + désenregistrement) : sans ça, un appareil déconnecté garderait le mdp wifi et les dates de séjours consultables hors ligne.
  - **Contenu recalculé côté client, jamais au rendu serveur** : le HTML est figé dans le cache le jour du précache, donc un rendu serveur afficherait les marées de ce jour-là comme celles d'aujourd'hui. Marées (grammaire identique à la bannière en ligne, **2 prochaines marées**), poubelles et rotation été réutilisent les composants et les libs existants plutôt que d'en recopier la logique.
  - **Calendrier : snapshot applicatif en lecture seule**, jamais un cache HTTP de réponses Supabase. Écrit depuis la page calendrier à partir des séjours **déjà chargés** par le serveur — zéro requête supplémentaire — et non plus à chaque ouverture du dashboard. Garde **tous** les séjours (166 de 2015 à 2027, ~45 Ko) : découper une fenêtre dans si peu de données ajoutait des cas limites sans rien économiser. Il réutilise la **vraie vue calendrier** de l'app avec des callbacks neutres. Bandeau à **trois états** (`lib/offline-snapshot.ts`, **12 tests**) : neutre en deçà de 7 jours, **avertissement au-delà**, « pas encore de synchro » si absent. Le calendrier N+1 étant vide et se remplissant sur ~6 mois, un snapshot périmé est *systématiquement optimiste* — il montre libre ce qui vient d'être pris.
  - **Profil hors ligne** : même principe que le calendrier — `/dashboard/profil` recopie en local ce qu'il a déjà chargé (nom, e-mail, rôles, séjours), sans requête. Dégradation en escalier : rien ouvert → invitation à se connecter ; calendrier ouvert → famille et priorité ; profil ouvert → tout. Ces champs personnels vivent dans le `localStorage` de l'appareil, jamais dans le HTML précaché qui est partagé, et sont purgés à la déconnexion.
  - `WIFI_PASSWORD` déplacé de `MaisonStatus.tsx` vers `lib/config.ts` (une constante, deux surfaces). Les helpers de sélection des marées (`upcomingTides`, `allTodayTides`, `timeToMinutes`) quittent `conditions.ts` — qui fait du réseau — pour `lib/tides-times.ts`, pur et désormais testé (**7 tests**) : les affichages en ligne et hors ligne ne peuvent plus diverger.
  - **Six pages hors ligne** : hub, calendrier, profil (famille + priorité été, calcul pur), à propos, règles, et le **guide de la télé** — dont le contenu est désormais partagé avec la version en ligne (`components/tele/TeleGuide.tsx`) pour qu'il n'y ait jamais deux guides à corriger.
  - **Sortie automatique** : `/hors-ligne` existant aussi côté serveur, actualiser une fois reconnecté reservait la page hors ligne, sans issue visible. `OfflineAutoExit` sonde le serveur (pas `navigator.onLine`, qui est `true` sur un wifi sans internet) et ramène vers **la page équivalente** en ligne.
  - **Budget de précache** : 810 Ko transférés, 2,8 Mo sur disque pour les six pages. Le poids vient des chunks JS — surtout du calendrier, qui réutilise la vraie vue de l'app (`date-holidays` compris). Les images du guide télé sont servies en copies réduites (`public/tele/offline/`) : 136 Ko au lieu de 471 pour un rendu identique sur téléphone ; la vidéo de démonstration (4,1 Mo) n'est jamais précachée.
  - Deux pièges de développement documentés dans `docs/guides/pieges-connus.md` (n°4 et n°5) : le mode hors ligne **ne se teste pas en `next dev`** (Turbopack), et le SW ne se met à jour qu'au rechargement.
...
- **Easter egg — idées de séjour dans le champ « Note »** : le placeholder de la note d'une nouvelle demande propose une activité du coin tirée au hasard (« Aller à la pêche aux lançons », « Voir le rayon vert depuis la plage du Val », « Faire du canoë jusqu'à Bénétin »…) au lieu du générique « Ex: Vacances de Noël en famille ». Liste dans `lib/idees-sejour.ts`, à enrichir librement. Tirage fait **après le montage** (`useEffect`) : le faire pendant le rendu donnerait une valeur serveur ≠ client, donc une erreur d'hydratation.
