...
- **#26 — Conditions du jour dans la bannière** : quand un séjour est en cours (cas A/B), la bannière contextuelle du dashboard s'enrichit de lignes « conditions », **dans son propre style** (pas de widget) : icônes d'accent, valeurs en gras, labels atténués, coef en pastille douce. Contenu :
  ...
  - Sources : **temp. eau** = API **Stormglass** (`lib/sea-temp.ts`, clé `STORMGLASS_API_KEY` en env Vercel, cache 6h ≈ 4 appels/j sous le quota gratuit de 10) ; **marées** (heures) = scraper `maree.info` ; **météo** = Open-Meteo Forecast (min/max, écart vs hier via `past_days=1`, coucher du soleil, tendance 7j) ; **coef** = statique `lib/tides.ts`. **Pas de fallback Open-Meteo Marine pour la temp. eau** : son modèle lit trop haut près des côtes (~19° vs ~13° réels).
    - *Historique des pistes temp. eau abandonnées* : Open-Meteo Marine (biaisé ~19°), puis scrapers letelegramme/Rothéneuf & lachainemeteo/Val (pages non récupérables pour caler le sélecteur), puis cabaigne/St-Malo (récupérable mais **403 en prod** : Cloudflare bloque l'IP datacenter de Vercel). D'où le passage à une API à clé.
