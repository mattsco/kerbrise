...
- **Tolérance « ≈ Période X » dans `getSummerSnapshot`** (issu de #39) : le matching des périodes d'été exige aujourd'hui les dates canoniques **exactes**. Constat sur les données réelles de l'été 2026 : **2 périodes sur 3 sont saisies à un jour près** (P1 28 au lieu du 29 juin, P3 jusqu'au 30 au lieu du 31 août) → les lignes juin/septembre de `PriorityCard` et les warnings #39 sont **silencieusement inertes** pour 2026. Chantier structurant : touche aussi la logique placeholder et l'auto-assignment (#22a), donc à trancher séparément (quelle tolérance ? ±2 j ? chevauchement majoritaire ?). C'est le prérequis pour que les règles juin/septembre vivent réellement.
