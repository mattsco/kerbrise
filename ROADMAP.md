...
- **#34 Tests `lib/` + CI minimale** — **avant la saison de planification**. Zéro test, zéro CI dans le repo, alors que la logique la plus critique (rotation été modulo 3, priorités P1/P2/P3, **modèle pivot 2027+ jamais exercé en réel**, `dates.ts`, indexation positionnelle des coefs, `db-errors`) est pure et sans I/O → testable en une demi-journée, et sollicitée à froid en nov–février. Vitest sur `lib/` uniquement (pas de tests UI ni E2E — théâtre à 14 users), GitHub Action `tsc --noEmit` + lint + tests (< 2 min, pas de secrets). Meilleur ratio sérénité/effort du repo. Spec `docs/specs/tests-ci-lib.md`.
...
- **#37 Offline PWA minimal** : la PWA installée n'a **pas de service worker** → écran blanc si le réseau tombe, alors que l'app acte elle-même que le wifi de la maison est faillible (carte Freebox, RPi #14) — et c'est précisément pendant une panne qu'on a besoin des infos pratiques. Cible : une page `/hors-ligne` précachée (marées du jour depuis les tables committées, poubelles, numéros utiles), rien d'autre — pas d'offline des flux Supabase. **Déclencheur** : une vraie plainte famille pendant une panne, ou des coupures fréquentes mesurées par le RPi #14 ; sinon on ne construit pas. Questions ouvertes (mdp wifi en cache clair, auth vs précache, invalidation SW, Safari iOS) : spec embryon `docs/specs/pwa-offline-minimal.md`.
