...
- **#37 Offline PWA minimal** : ✅ **implémenté le 21 juillet 2026**, en attente de validation preview + iPhone avant merge. Surface hors ligne calquée sur l'app (`/hors-ligne` + calendrier, à-propos, règles), service worker écrit à la main sans dépendance, snapshot calendrier lecture seule avec bandeau de fraîcheur à 3 états. Reste bloquant avant merge : **test sur un vrai iPhone** (installation PWA, mode avion, éviction de stockage). Spec `docs/specs/pwa-offline-minimal.md`.
